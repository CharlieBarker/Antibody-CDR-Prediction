...
else
{
       ...
       !defined($::noloopdb))
    {
        ...
        $mod3, my @loopInfo = RunLoopdb($mod2, $::loophit, $targetCanonicals{'H3'});
        print STDERR "output -> $mod3\n";
        print STDERR "lOOP LENGTH! -> $targetCanonicals{'H3'}\n";
        my $trash = shift @loopInfo;
        my $name = shift @loopInfo;
        print STDERR "name -> $name\n";
        print STDERR "top E -> @loopInfo\n";
        print STDERR "done\n" if($::v >= 1);
	#write file containing this information in tab seperated format if all values exist
	if($name && @loopInfo && $targetCanonicals{'H3'})
	{
		...
		else 
		{
			`touch loopinfo.txt`;
			open (MYFILE, ">>$config::analysisResults/loopinfo.txt");
			print MYFILE "Name\tLoopsize\tEnergy\n";
			print MYFILE "$name\t$targetCanonicals{'H3'}\t@loopInfo\n";
 			close (MYFILE);
		}
	}
    }
...
sub RunLoopdb
{
    ...
    my(@hits) = SortSplicedLoops($inPDB, 'H3', @hits);
    my $topEnergy = pop @hits;
    ...
    my $name = $hits[0]; 
    my @loopInfo = ($name, $topEnergy);
    return($outPDB, @loopInfo);
}

#*************************************************************************
#> @hits = SortSplicedLoops($inPDB, 'H3', @hits)
#  ---------------------------------------------
sub SortSplicedLoops
{
    ...
    my $firstPos = $idx[0];
    # Get the energy of the top hit
    my $firstEnergy = $energies[$firstPos];
    #don't know why, but this is the only thing that works. @sortedHits 
    #contains the sorted hits, plus the energy on the end. 
    push @sortedHits, $firstEnergy;
    return(@sortedHits);
}
...
sub SpliceLoop
{
    ...
    # splicepdb V2.x 
    my $exe = "$config::bindir/splicepdb -l";
...
abYmod V1.14 buildmodel (c) 2013-2015, Dr. Andrew C.R. Martin, UCL
